...
def plot_experiments2(prefixes, exp_ids, rename_dict=None, stat_type='objective'):
    if not rename_dict:
        rename_dict = dict()
    for p, e in zip(prefixes, exp_ids):
        evals, lower, mean, upper = get_plot_data(p, e, stat_type)
        plot_experiment(evals, lower, mean, upper, rename_dict.get(e, e))
    plt.legend()
    plt.xlabel('Fitness evaluations')
    if stat_type == 'objective':
