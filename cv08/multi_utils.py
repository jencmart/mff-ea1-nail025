...
def assign_ssc_crowding_distances(front):
...
def assign_ssc_hyper_volume(front, ref=HYP_REF):
    f1_sorted = list(sorted(front, key=lambda x: x.fitness[0]))
    for (i, j) in zip(f1_sorted, f1_sorted[1:]):
        hyper_vol_for_i = (ref[1] - i.fitness[1]) * (j.fitness[0] - i.fitness[0])
        i.ssc = hyper_vol_for_i
    hyper_vol_for_last = (ref[1] - f1_sorted[-1].fitness[1]) * (ref[0] - f1_sorted[-1].fitness[0])
    f1_sorted[-1].ssc = hyper_vol_for_last
