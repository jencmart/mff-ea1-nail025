...
def hypervolume(pop, ref=HYP_REF):
    ...
    f1_sorted = list(sorted(non_dom, key=lambda x: x.fitness_minmax[0]))

    volume = 0

    for (i, j) in zip(f1_sorted, f1_sorted[1:]):
        volume += (ref[1] - i.fitness_minmax[1]) * (j.fitness_minmax[0] - i.fitness_minmax[0])
    
    volume += (ref[1] - f1_sorted[-1].fitness_minmax[1]) * (ref[0] - f1_sorted[-1].fitness_minmax[0])
...
def assign_crowding_distances(front):
    ...
    for i in range(1, len(front) - 1):
        front[i].ssc = (front[i + 1].fitness_minmax[0] - front[i - 1].fitness_minmax[0] +
                        front[i - 1].fitness_minmax[1] - front[i + 1].fitness_minmax[1])
...
def get_first_nondominated(pop):
    ...
    for i, p in enumerate(pop):
        if not any(map(lambda x: dominates(x.fitness_minmax, pop[i].fitness_minmax), pop)):
            non_dom.append(i)
    return non_dom
